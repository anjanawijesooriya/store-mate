...
export async function GET() {
  try {
    ...
    const [shop, device] = await Promise.all([
      db.shop.findUnique({ where: { id: shopId }, select: { branchModeEnabled: true } }),
    ...
    const branchModeEnabled = shop?.branchModeEnabled ?? false;
    const isPrimary = device?.isPrimary ?? false;

    return Response.json({ branchModeEnabled, isPrimary });
  } catch (err) {
